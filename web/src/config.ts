/**
 * DPP enrichment configuration.
 * Non-EPD properties (mechanical, DoPC) are hardcoded here.
 * EPD indicator data is derived from the JSON-LD source files
 * via epd-data.ts — single source of truth.
 */

import { getEpdProperties, getEpdIndicators } from './epd-data';
...
export const COMPONENTS: Record<string, ComponentConfig> = {
  insulation: {
    ...
    properties: [
      ...
      // EPD indicators — derived from JSON-LD source (single source of truth)
      ...getEpdProperties('insulation'),
    ],
    documents: [
...
export const LCA_COMPONENTS: Record<string, LcaComponentConfig> = {
  insulation: {
    ...
    referenceQuantity: 1,
    // EPD indicators — derived from JSON-LD source (single source of truth)
    indicators: getEpdIndicators('insulation'),
  },
  timber: {
    label: 'Timber (Schilliger Glulam GL24h)',
    shortLabel: 'Timber',
    ...
    referenceQuantity: 1,
    // EPD indicators — derived from JSON-LD source (single source of truth)
    indicators: getEpdIndicators('timber'),
  },
  pipe: {
    label: 'Pipe (Wavin PVC DN110)',
    shortLabel: 'Pipe',
    ...
    linearDensity: 1.58,
    // EPD indicators — derived from JSON-LD source (single source of truth)
    indicators: getEpdIndicators('pipe'),
  },
};
